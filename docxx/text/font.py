...
from docxx.dml.color import ColorFormat
...
#
# 16/05/07
# <w:em>    | EmFormat, Font.em (font.py), CT_Em, CT_RPr.em (oxml/text/font.py), register (__init__.py)
# <w:ruby>  | Ruby, Run.ruby (run.py), CT_Ruby, CT_R.ruby (oxml/text/run.py), register (__init__.py)
#
class EmFormat(ElementProxy):   
    """
    Provides access to <w:em>
    character:
    ...
    def __init__(self, rPr_parent):
        super(EmFormat, self).__init__(rPr_parent)
    ...
    def character(self, value):
        if value is None and self._em is None:
            return
        rPr = self._element.get_or_add_rPr()
        rPr._remove_em()
        rPr.get_or_add_em().val = value
...
class Font(ElementProxy):
    ...
    def size(self):
        ...
        length units. The :class:`docxx.shared.Pt` class allows convenient
