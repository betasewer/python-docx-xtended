...
from docxx.enum.section import WD_HEADER_FOOTER, WD_ORIENTATION, WD_SECTION_START
from docxx.oxml.simpletypes import ST_SignedTwipsMeasure, ST_TwipsMeasure, ST_TextDirection, ST_DocGrid, ST_DecimalNumber, XsdString
...
class CT_SectPr(BaseOxmlElement):
    ...
    textDirection = ZeroOrOne("w:textDirection", successors=_tag_seq[15:])
    docGrid = ZeroOrOne("w:docGrid", successors=_tag_seq[18:])
    del _tag_seq
    ...
    def grid_type(self):
        grid = self.docGrid
        return grid.type if grid is not None else None
...
class CT_TextDirection(BaseOxmlElement):
    ...
    ``<w:textDirection>``
    """
    val = OptionalAttribute('w:val', ST_TextDirection)
