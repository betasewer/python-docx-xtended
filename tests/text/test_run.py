...
from docxx.enum.text import WD_BREAK, WD_UNDERLINE
...
class DescribeRun(object):
    ...
    def Font_(self, request, font_):
        return class_mock(request, 'docxx.text.run.Font', return_value=font_)
    ...
    def InlineShape_(self, request):
        return class_mock(request, 'docxx.text.run.InlineShape')
    ...
    def Text_(self, request):
        return class_mock(request, 'docxx.text.run._Text')
