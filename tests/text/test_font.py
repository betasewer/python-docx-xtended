...
"""
Test suite for the docxx.text.run module
"""
...
from docxx.enum.text import WD_COLOR, WD_UNDERLINE
from docxx.shared import Pt
...
class DescribeFont(object):
    ...
    def it_knows_its_typeface_name(self, name_get_fixture):
        ...
        assert font.ansiname == expected_value
    ...
    @pytest.fixture(params=[
        ('w:r',                                          'Foo',
         'w:r/w:rPr/w:rFonts{w:ascii=Foo,w:hAnsi=Foo,w:hint=default}'),
        ('w:r/w:rPr',                                    'Foo',
         'w:r/w:rPr/w:rFonts{w:ascii=Foo,w:hAnsi=Foo,w:hint=default}'),
        ('w:r/w:rPr/w:rFonts{w:hAnsi=Foo}',              'Bar',
         'w:r/w:rPr/w:rFonts{w:ascii=Bar,w:hAnsi=Bar,w:hint=default}'),
        ('w:r/w:rPr/w:rFonts{w:ascii=Foo,w:hAnsi=Foo,w:hint=default}',  'Bar',
         'w:r/w:rPr/w:rFonts{w:ascii=Bar,w:hAnsi=Bar,w:hint=default}'),
    ])
    ...
    def ColorFormat_(self, request, color_):
        return class_mock(
            request, 'docxx.text.font.ColorFormat', return_value=color_
        )
