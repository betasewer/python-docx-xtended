...
"""Unit test suite for docxx.image.jpeg module"""

from __future__ import absolute_import, print_function

import pytest

from docxx.compat import BytesIO
from docxx.image.constants import JPEG_MARKER_CODE, MIME_TYPE
from docxx.image.helpers import BIG_ENDIAN, StreamReader
from docxx.image.jpeg import (
    _App0Marker,
    _App1Marker,
    Exif,
    Jfif,
...
from docxx.image.tiff import Tiff
...
class DescribeJpeg(object):
    ...
    def _JfifMarkers_(self, request, jfif_markers_):
        _JfifMarkers_ = class_mock(request, 'docxx.image.jpeg._JfifMarkers')
...
class Describe_JfifMarkers(object):
    ...
    def _MarkerParser_(self, request, marker_parser_):
        _MarkerParser_ = class_mock(request, 'docxx.image.jpeg._MarkerParser')
...
class Describe_App1Marker(object):
    ...
    def BytesIO_(self, request, substream_):
        return class_mock(
            request, 'docxx.image.jpeg.BytesIO', return_value=substream_
        )
    ...
    def Tiff_(self, request, tiff_):
        Tiff_ = class_mock(request, 'docxx.image.jpeg.Tiff')
...
class Describe_MarkerFactory(object):
    ...
    def _App0Marker_(self, request):
        return class_mock(request, 'docxx.image.jpeg._App0Marker')

    @pytest.fixture
    def _App1Marker_(self, request):
        return class_mock(request, 'docxx.image.jpeg._App1Marker')

    @pytest.fixture
    def _Marker_(self, request):
        return class_mock(request, 'docxx.image.jpeg._Marker')
    ...
    def _SofMarker_(self, request):
        return class_mock(request, 'docxx.image.jpeg._SofMarker')
...
class Describe_MarkerParser(object):
    ...
    def _MarkerFactory_(self, request, soi_, app0_, eoi_):
        return class_mock(
            request, 'docxx.image.jpeg._MarkerFactory',
            side_effect=[soi_, app0_, eoi_]
        )

    @pytest.fixture
    def _MarkerFinder_(self, request, marker_finder_):
        _MarkerFinder_ = class_mock(request, 'docxx.image.jpeg._MarkerFinder')
    ...
    def StreamReader_(self, request, stream_reader_):
        return class_mock(
            request, 'docxx.image.jpeg.StreamReader',
