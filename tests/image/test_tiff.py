...
"""Unit test suite for docxx.image.tiff module"""
...
from docxx.image.constants import MIME_TYPE, TIFF_TAG
from docxx.image.helpers import BIG_ENDIAN, LITTLE_ENDIAN, StreamReader
from docxx.image.tiff import (
    _AsciiIfdEntry,
    _IfdEntries,
    _IfdEntry,
    _IfdEntryFactory,
...
class DescribeTiff(object):
    ...
    def _TiffParser_(self, request, tiff_parser_):
        _TiffParser_ = class_mock(request, 'docxx.image.tiff._TiffParser')
...
class Describe_TiffParser(object):
    ...
    def _IfdEntries_(self, request, ifd_entries_):
        _IfdEntries_ = class_mock(request, 'docxx.image.tiff._IfdEntries')
    ...
    def StreamReader_(self, request, stream_rdr_):
        return class_mock(
            request, 'docxx.image.tiff.StreamReader', return_value=stream_rdr_
        )
...
class Describe_IfdEntries(object):
    ...
    def _IfdParser_(self, request, ifd_parser_):
        return class_mock(
            request, 'docxx.image.tiff._IfdParser', return_value=ifd_parser_
        )
...
class Describe_IfdParser(object):
    ...
    def _IfdEntryFactory_(self, request, ifd_entry_, ifd_entry_2_):
        return function_mock(
            request, 'docxx.image.tiff._IfdEntryFactory',
...
class Describe_IfdEntryFactory(object):
    ...
    def _IfdEntry_(self, request, ifd_entry_):
        _IfdEntry_ = class_mock(request, 'docxx.image.tiff._IfdEntry')
    ...
    def _AsciiIfdEntry_(self, request, ifd_entry_):
        _AsciiIfdEntry_ = class_mock(
            request, 'docxx.image.tiff._AsciiIfdEntry')
    ...
    def _ShortIfdEntry_(self, request, ifd_entry_):
        _ShortIfdEntry_ = class_mock(
            request, 'docxx.image.tiff._ShortIfdEntry')
    ...
    def _LongIfdEntry_(self, request, ifd_entry_):
        _LongIfdEntry_ = class_mock(
            request, 'docxx.image.tiff._LongIfdEntry')
    ...
    def _RationalIfdEntry_(self, request, ifd_entry_):
        _RationalIfdEntry_ = class_mock(
            request, 'docxx.image.tiff._RationalIfdEntry')
