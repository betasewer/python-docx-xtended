...
"""Unit test suite for docxx.image.png module"""
...
from docxx.image.constants import MIME_TYPE, PNG_CHUNK_TYPE
from docxx.image.exceptions import InvalidImageStreamError
from docxx.image.helpers import BIG_ENDIAN, StreamReader
from docxx.image.png import (
...
class DescribePng(object):
    ...
    def _PngParser_(self, request, png_parser_):
        _PngParser_ = class_mock(request, 'docxx.image.png._PngParser')
...
class Describe_PngParser(object):
    ...
    def _Chunks_(self, request, chunks_):
        _Chunks_ = class_mock(request, 'docxx.image.png._Chunks')
...
class Describe_Chunks(object):
    ...
    def _ChunkParser_(self, request, chunk_parser_):
        _ChunkParser_ = class_mock(request, 'docxx.image.png._ChunkParser')
...
class Describe_ChunkParser(object):
    ...
    def _ChunkFactory_(self, request, chunk_lst_):
        return function_mock(
            request, 'docxx.image.png._ChunkFactory',
    ...
    def StreamReader_(self, request, stream_rdr_):
        return class_mock(
            request, 'docxx.image.png.StreamReader', return_value=stream_rdr_
        )
...
class Describe_ChunkFactory(object):
    ...
    def _Chunk_(self, request, chunk_):
        _Chunk_ = class_mock(request, 'docxx.image.png._Chunk')
    ...
    def _IHDRChunk_(self, request, ihdr_chunk_):
        _IHDRChunk_ = class_mock(request, 'docxx.image.png._IHDRChunk')
    ...
    def _pHYsChunk_(self, request, phys_chunk_):
        _pHYsChunk_ = class_mock(request, 'docxx.image.png._pHYsChunk')
