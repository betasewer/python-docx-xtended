...
"""
Test suite for the docxx.api module
"""
...
from docxx.api import Document, open_docx, compose_docx
from docxx.opc.constants import CONTENT_TYPE as CT

from .unitutil.mock import function_mock, instance_mock, class_mock

# Document -> open_docxへの対応は今後
# compose_docxのテストも追加すること
class DescribeDocument(object):

    def __SKIPPED_it_opens_a_docx_file(self, open_fixture):
        docx, Package_, document_ = open_fixture
        document = Document(docx)
        Package_.open.assert_called_once_with(docx)
        assert document is document_

    def __SKIPPED_it_opens_the_default_docx_if_none_specified(self, default_fixture):
        docx, Package_, document_ = default_fixture
        document = Document()
        Package_.open.assert_called_once_with(docx)
        assert document is document_

    def __SKIPPED_it_raises_on_not_a_Word_file(self, raise_fixture):
    ...
    def _default_docx_path_(self, request):
        return function_mock(request, 'docxx.api._default_docx_path')

    @pytest.fixture
    def document_(self, request):
        return instance_mock(request, docxx.document.Document)

    @pytest.fixture
    def Package_(self, request):
        return class_mock(request, 'docxx.api.Package')
