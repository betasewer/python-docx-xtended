...
"""Unit test suite for docxx.opc.pkgreader module"""

from __future__ import absolute_import, print_function, unicode_literals

import pytest

from docxx.opc.constants import CONTENT_TYPE as CT, RELATIONSHIP_TARGET_MODE as RTM
from docxx.opc.packuri import PackURI
from docxx.opc.phys_pkg import _ZipPkgReader
from docxx.opc.pkgreader import (
    _ContentTypeMap,
    PackageReader,
    _SerializedPart,
...
class DescribePackageReader(object):
    ...
    def PhysPkgReader_(self, request):
        _patch = patch(
            'docxx.opc.pkgreader.PhysPkgReader', spec_set=_ZipPkgReader
        )
    ...
    def _SerializedPart_(self, request):
        return class_mock(request, 'docxx.opc.pkgreader._SerializedPart')

    @pytest.fixture
    def _SerializedRelationships_(self, request):
        return class_mock(
            request, 'docxx.opc.pkgreader._SerializedRelationships'
        )
...
class Describe_SerializedRelationships(object):
    ...
    def parse_xml_(self, request):
        return function_mock(request, 'docxx.opc.pkgreader.parse_xml')

    @pytest.fixture
    def _SerializedRelationship_(self, request):
        return class_mock(
            request, 'docxx.opc.pkgreader._SerializedRelationship'
        )
